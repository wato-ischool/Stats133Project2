...
Next, we created the `processLine` function which will process the data and create a character matrix with 10 columns from a line of the data file. This function consisted of using regular expressions, mainly `gsub`, for parsing the data. 

`gsub` takes the arguments of a regular expression, a replacement, and a string, and then replaces the original argument with the replacement argument. Sometimes, `.*` is insufficient to capture a group you want because it is too greedy. To make the regular expression "lazy" (or not "greedy"), add a question mark (`?`). To parse the data, we ignored everything before `t=` appears in the string, captured the text between `t=` and `;`, and ignored everything after the `;`. We did the same for `scanMac`, `pos`, and `degree` as well. We split `pos` into the `x`, `y`, and `z` components. This graph shows only the `x` and `y` components since `z` is zero for all observations.


```{r, echo=FALSE}
load("project2.RData")

plot(x = offline$posX, y = offline$posY, type = "p",
     main = "X and Y", xlab = "x", ylab = "y")
```
...
First we listed out all MAC addresses and noticed that there were many ones.

```{r, echo=FALSE}
expdata = table(offline2$mac)
expdataf = data.frame(expdata)
as.numeric(expdataf$Freq)

barplot(expdataf$Freq, names.arg= expdataf$Var1, cex.names=.25, col = "slateblue1", 
        main = "Frequency for All Mac Addresses", ylab = "number of occurrences", 
        xlab = "Mac Addresses", axes=TRUE, axisnames=TRUE)
```
  
Next we removed the frequencies of ones because they are on different floors.

```{r, echo=FALSE}
expdataf2 = expdataf[expdataf$Freq > 1, ]
as.numeric(expdataf2$Freq)

barplot(expdataf2$Freq, names.arg= expdataf2$Var1, cex.names=.5, col = "slateblue2", 
        main = "Frequency for Mac Addresses Greater than One", 
...
expdataf3 = expdataf3[expdataf3$Var1 != "00:0f:a3:39:dd:cd", ]
...
barplot(expdataf3$Freq, names.arg= expdataf3$Var1, cex.names=.75, col = c("red", "red", "slateblue3", "slateblue3", "slateblue3", "slateblue3", "slateblue3"), 
        main = "Frequency for Potential Mac Addresses Greater than 110,000", 
        ylab = "number of occurences", xlab = "Mac Addresses", 
        axes=TRUE, axisnames=TRUE)
```
...
4. Drop `type` which can be computed from `mac`. (The `mac`'s type is 3 for access point and 1 for device in adhoc mode.)
...
For part 3, we made `combineSignals` to combine the data by coordinates because each 6 have the same `(x, y)` coordinates based on how the data was cleaned. First we removed data that didn't fit and made an index for every 6th row which we kept. Next we found the nearest neighbor by using Euclidean distance equation provided in the documentation with the groups from the combined coordinates. 

Lastly, we calculated errors using the Euclidean distance again. Here we did the nearest neighbor computations and the cross validation and came up with the average errors for each `k` value.
